class ProjectController < ApplicationController
  ...
  def assign_user
    if @project.users.include?(User.find(params[:user_id]))
      ...
      @project.users.each do |project_user|
        ...
        UserMailer.assignment_email(project_user, @project, current_user).deliver_later
      end
  ...
  def project_params
    params.require(:project).permit(:title, :description, :start_date, :end_date, :content, :client_id, :software_id, :user_id)
  end
end
